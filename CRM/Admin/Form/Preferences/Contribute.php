...
class CRM_Admin_Form_Preferences_Contribute extends CRM_Admin_Form_Preferences {
  ...
  public function buildQuickForm() {
    ...
    foreach ($this->_settings as $setting => $group) {
      $settingMetaData = civicrm_api3('setting', 'getfields', ['name' => $setting]);
      $props = $settingMetaData['values'][$setting];
      if (isset($props['quick_form_type'])) {
        $add = 'add' . $props['quick_form_type'];
        if ($add == 'addElement') {
          if (in_array($props['html_type'], ['checkbox', 'textarea'])) {
            ...
            if ($props['html_type'] == 'select') {
              ...
              if ($functionName) {
                $props['option_values'] = ['' => ts('- select -')] + CRM_Contribute_PseudoConstant::$functionName();
              }
            }
            $this->$add(
              $props['html_type'],
              $setting,
              ts($props['title']),
              CRM_Utils_Array::value($props['html_type'] == 'select' ? 'option_values' : 'html_attributes', $props, []),
        ...
        elseif ($add == 'addDate') {
          $this->addDate($setting, ts($props['title']), FALSE, ['formatType' => $props['type']]);
        }
        else {
  ...
  public function setDefaultValues() {
    ...
    foreach (['cvv_backoffice_required'] as $setting) {
  ...
  public function postProcess() {
    ...
    if (isset($params['invoicing'])) {
      $value = [$setKey => $optionValues[$setKey]];
    ...
    //CRM-16691: Changes made related to settings of 'CVV'.
    $settings = array_intersect_key($params, ['cvv_backoffice_required' => 1]);
