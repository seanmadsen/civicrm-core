...
class CRM_Member_Form_MembershipRenewal extends CRM_Member_Form {
  ...
  public function buildQuickForm() {
    ...
    foreach ($this->allMembershipTypeDetails as $key => $values) {
      if (!empty($values['is_active'])) {
        ...
        $allMembershipInfo[$key] = [
          ...
          'tax_message' => $taxAmount ? ts("Includes %1 amount of %2", [1 => CRM_Utils_Array::value('tax_term', $invoiceSettings), 2 => CRM_Utils_Money::format($taxAmount)]) : $taxAmount,
        ];
    ...
    $this->add('text', 'num_terms', ts('Extend Membership by'), ['onchange' => "setPaymentBlock();"], TRUE);
