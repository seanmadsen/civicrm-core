...
class CRM_Member_BAO_MembershipType extends CRM_Member_DAO_MembershipType {
  ...
  public static function getMembershipTypesByOrg($orgID) {
    Civi::log()->warning('Deprecated function getMembershipTypesByOrg, please user membership_type api', ['civi.tag' => 'deprecated']);
  ...
  public static function updateAllPriceFieldValue($membershipTypeId, $params) {
    ...
    $updateValues = [
      2 => ['financial_type_id', 'financial_type_id', 'Integer'],
      3 => ['label', 'name', 'String'],
      4 => ['amount', 'minimum_fee', 'Float'],
      5 => ['description', 'description', 'String'],
    ];

    $queryParams = [1 => [$membershipTypeId, 'Integer']];
    foreach ($updateValues as $key => $value) {
      if (array_key_exists($value[1], $params)) {
        ...
        $queryParams[$key] = [$fieldValue, $value[2]];
      }
    }
