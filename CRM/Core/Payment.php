...
abstract class CRM_Core_Payment {
  ...
  public function getText($context, $params) {
    ...
    switch ($context) {
      case 'contributionPageRecurringHelp':
        ...
        if (array_keys($params) == ['is_recur_installments', 'is_email_receipt']) {
  ...
  public static function handlePaymentMethod($method, $params = []) {
    ...
    if (!$extension_instance_found) {
      ...
      CRM_Core_Error::fatal(ts($message, [1 => $params['processor_name'], 2 => $method]));
    }
  }
...
INNER JOIN civicrm_contribution con ON ( con.contribution_recur_id = rec.id )
     WHERE rec.id = %1";
          $contactID = CRM_Core_DAO::singleValueQuery($sql, [1 => [$entityID, 'Integer']]);
          $entityArg = 'crid';
          break;
      }
    }
  ...
  protected function getPaymentDescription($params, $length = 24) {
    $parts = ['contactID', 'contributionID', 'description', 'billing_first_name', 'billing_last_name'];
    $validParts = [];
    if (isset($params['description'])) {
      $uninformativeStrings = [ts('Online Event Registration: '), ts('Online Contribution: ')];
