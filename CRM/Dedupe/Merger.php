  ...
  public static function getMultiValueCustomSets($request) {
    ...
    if (!isset(Civi::$statics[__CLASS__]['multiValueCustomSets'])) {
      ...
      $result = civicrm_api3('custom_group', 'get', [
        'is_multiple' => 1,
        'extends' => ['IN' => ['Individual', 'Organization', 'Household', 'Contact']],
...
INNER JOIN  civicrm_membership membership2 ON membership1.membership_type_id = membership2.membership_type_id
   ...
   * @return array|bool
   */
  public static function batchMerge($rgid, $gid = NULL, $mode = 'safe', $batchLimit = 1, $isSelected = 2, $criteria = [], $checkPermissions = TRUE) {
  ...
  public static function getMergeStatsMsg($stats) {
    ...
    if (!empty($stats['merged'])) {
      $msg = '<p>' . ts('One contact merged.', ['count' => $stats['merged'], 'plural' => '%count contacts merged.']) . '</p>';
    }
    if (!empty($stats['skipped'])) {
      $msg .= '<p>' . ts('One contact was skipped.', ['count' => $stats['skipped'], 'plural' => '%count contacts were skipped.']) . '</p>';
    }
    return $msg;
  }
  ...
  static public function locationIsSame($mainAddress, $comparisonAddress) {
    $keysToIgnore = ['id', 'is_primary', 'is_billing', 'manual_geo_code', 'contact_id', 'reset_date', 'hold_date'];
  ...
  public static function getRowsElementsAndInfo($mainId, $otherId, $checkPermissions = TRUE) {
    ...
    foreach ($locationBlocks as $blockName => $blockInfo) {
      ...
      if (!empty($locations['other']) && !empty($locations['other'][$blockName])) {
        foreach ($locations['other'][$blockName] as $count => $value) {
          ...
          $elements[] = ['advcheckbox', "location_blocks[$blockName][$count][set_other_primary]", NULL, ts('Set as primary')];
          ...
          $elements[] = ['hidden', "location_blocks[$blockName][$count][mainContactBlockId]", $mainContactBlockId];
          ...
          if ($blockInfo['hasLocation']) {
            ...
            if ($blockName != 'address') {
              $elements[] = ['advcheckbox', "location_blocks[{$blockName}][$count][operation]", NULL, ts('Add new')];
    ...
    foreach ($relTables as $name => $null) {
      ...
      if ($name == 'rel_table_memberships') {
        $elements[] = ['checkbox', "operation[move_{$name}][add]", NULL, ts('add new')];
  ...
  public static function moveAllBelongings($mainId, $otherId, $migrationInfo, $checkPermissions = TRUE) {
    ...
    $names['communication_style'] = ['newName' => 'communication_style_id', 'groupName' => 'communication_style'];
  ...
  public static function createMergeActivities($mainId, $otherId) {
    ...
    if (civicrm_api3('Setting', 'getvalue', ['name' => 'contact_undelete', 'group' => 'CiviCRM Preferences'])) {
  ...
  public static function getDuplicatePairs($rule_group_id, $group_id, $reloadCacheIfEmpty, $batchLimit, $isSelected, $orderByClause = '', $includeConflicts = TRUE, $criteria = [], $checkPermissions = TRUE, $searchLimit = 0) {
    ...
    $join = self::getJoinOnDedupeTable();
    $dupePairs = CRM_Core_BAO_PrevNextCache::retrieve($cacheKeyString, $join, $where, 0, 0, [], $orderByClause, $includeConflicts);
