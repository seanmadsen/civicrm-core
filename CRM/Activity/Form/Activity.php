...
class CRM_Activity_Form_Activity extends CRM_Contact_Form_Task {
  ...
  public function preProcess() {
    ...
    if (isset($activityTName)) {
      ...
      $this->assign('pageTitle', ts('%1 Activity', [1 => $activityName]));
  ...
  public function buildQuickForm() {
    ...
    $this->addDateTime('activity_date_time', ts('Date'), TRUE, ['formatType' => 'activityDateTime']);

    // Add followup date.
    $this->addDateTime('followup_date', ts('in'), FALSE, ['formatType' => 'activityDateTime']);
    ...
    if (!empty($tags)) {
      $this->add('select2', 'tag', ts('Tags'), $tags, FALSE, ['class' => 'huge', 'placeholder' => ts('- select -'), 'multiple' => TRUE]);
    }
    ...
    if ($this->_action & CRM_Core_Action::VIEW) {
      ...
      $js = ['onclick' => "return activityStatus(" . json_encode($message) . ");"];
      $this->addButtons([
        [
          'type' => 'upload',
          'name' => ts('Save'),
