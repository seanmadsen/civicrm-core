    ...
    foreach ($formValues as $id => &$val) {
      ...
      elseif ($id == 'email_on_hold') {
        if ($formValues['email_on_hold']['on_hold']) {
          $params[] = ['on_hold', '=', $formValues['email_on_hold']['on_hold'], 0, 0];
        }
      }
...
";
  ...
  public function relationship(&$values) {
    ...
    $rTypeValues = $relTypes = $relTypesIds = [];
...
civicrm_relationship.is_permission_a_b = 0
  ...
  public static function getGroupByFromOrderBy(&$groupBy, $orderBys) {
    if (!CRM_Utils_SQL::disableFullGroupByMode()) {
      foreach ($orderBys as $orderBy) {
        $orderBy = str_replace([' DESC', ' ASC', '`'], '', $orderBy); // remove sort syntax from ORDER BY clauses if present
  ...
  public function getCachedContacts($cacheKey, $offset, $rowCount, $includeContactIds) {
    ...
    $groupByCol = ['contact_a.id', 'pnc.id'];
      ...
      default:
        ...
        if (is_array($value)) {
          ...
          if (!empty($value[0])) {
            ...
            if (($queryString = CRM_Core_DAO::createSqlFilter($field, [$op => array_keys($value)], $dataType)) != FALSE) {
              return $queryString;
            }
          }
        }
...
";
  ...
  protected function prepareOrderBy($sort, $sortByChar, $sortOrder, $additionalFromClause) {
    ...
    if ($config->includeOrderByClause ||
      ...
      elseif ($sortByChar) {
        $orderByArray = ["UPPER(LEFT(contact_a.sort_name, 1)) asc"];
      }
      else {
